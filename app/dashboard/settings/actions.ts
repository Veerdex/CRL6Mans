...
export async function requestProfileChange(
  ...
  const peak1v1    = (formData.get("peak_1v1")    as string)?.trim();
  const current1v1 = (formData.get("current_1v1") as string)?.trim();
  ...
  if (Number(current1v1) > Number(peak1v1)) return { error: "Current 1v1 MMR cannot exceed Peak 1v1." };

  const { data: player } = await supabaseAdmin
    .from("players")
    .select("id, username, status, kick_reason, kicked_until, tracker_url, peak_3v3, current_3v3, peak_2v2, current_2v2, peak_1v1, current_1v1")
  ...
  const unchanged =
    ...
    Number(peak1v1)    === Number(player.peak_1v1 ?? 0) &&
    Number(current1v1) === Number(player.current_1v1 ?? 0);
