...
export default async function AdminPage() {
  ...
  const subRequestCards: SubRequestCardData[] = subReqs.map(req => {
    ...
    const subCandidates = candidateIds
      .map(id => subInMap[id])
      .filter(Boolean)
      .map(p => ({ discordId: p.discord_id ?? null, username: p.username, displayName: p.display_name ?? null, mmr: subPeakMmr(p) }));
    ...
    return {
      ...
      requestedByDiscordId: requester?.discord_id ?? null,
      ...
      {userIsDirector && (
        ...
        >
          <LeagueControls
            ...
            seasonAccoladePrizes={Object.fromEntries(
              SEASON_ACCOLADES.map(a => [a.key, ((settings as Record<string, unknown> | null)?.[a.prizeColumn] as number | null) ?? null]),
            )}
