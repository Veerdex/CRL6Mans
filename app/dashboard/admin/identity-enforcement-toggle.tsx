...
import { setIdentityEnforcementEnabled } from "./identity-discrepancy-actions";

export function IdentityEnforcementToggle({ initialEnabled }: { initialEnabled: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3">
      <div>
        <p className="text-sm font-medium text-white">Identity Enforcement</p>
        <p className="text-xs text-zinc-500 mt-0.5">
          {enabled
            ? "ON — matches with unresolved discrepancies are blocked from completing"
            : "OFF — discrepancies are recorded for review but never block match completion"}
        </p>
      ...
      <button
        onClick={() => {
          ...
          startTransition(async () => {
            const res = await setIdentityEnforcementEnabled(next);
            if (res.error) {
              setEnabled(!next);
              setError(res.error);
            }
