...
- Every message the bot posts during a draft is an embed built in `app/lib/draft-embeds.ts` — the same kind of pure leaf, tested by `npm run test:draft-embeds`. **A mention inside an embed does not notify anyone**, so every captain ping lives in the message `content` and the embed beside it carries plain text only; that is why the start announcement is two messages. The captain list chunks across fields at Discord's 1024-character limit, because an over-long field 400s the whole message instead of truncating.
- **A pick is one message, not two.** The on-the-clock message is edited into the result when the pick lands, so the channel ends up as one line per pick instead of a ping and a result stacked. `league_settings.draft_pick_message_id` holds the message currently on the clock; `rememberPickMessage` writes it, and it is released in the same update that advances `current_pick` so a racing autopick can't overwrite the winner's announcement. A timeout is the same edit in amber (`pickEmbed({auto: true})`), not a separate post — so a timed-out pick that then fails leaves the message reading as still on the clock. If the edit fails the result is posted instead, and `setDraftChannelVisibility(false)` clears the stored ID, since a stale one would rewrite a message in the previous draft's history. The column read degrading to `undefined` is the pre-migration fallback: every pick posts its own message, as it did before.
...
The crowning announcement is built in `app/lib/clip-embeds.ts` (a pure leaf, same convention as `draft-embeds.ts`). The registered-role ping lives in the message `content`, never the embed — a mention inside an embed notifies nobody — with `allowed_mentions: { parse: [], roles: [roleId] }`, since a clip title is player-supplied and would otherwise ping the server through the bot. `clips.clip_of_week_number` is a running count that never resets, stored on the winning clip so past winners keep their number; a read error on that column means the migration hasn't been run and the heading drops the week rather than failing the update that archives the winner. The embed's image never comes from `clips.thumbnail_url` — that column is only populated for the link-only platforms, which are exactly the ones barred from winning, so it is null for every possible winner. YouTube resolves offline through `clipPreviewImageUrl` (pure, from the video ID); Medal, Streamable and Twitch fall through to `fetchClipThumbnail`, which scrapes `og:image` because Medal's oEmbed endpoint is gone (410). **That scrape must stay on the crowning path, not the submission path**: Medal's `og:image` is a signed CDN URL that expires in about a day (the unsigned URL 403s) and a clip can sit in the pool for over a week, so a URL captured at submission is always dead by the time it wins. Discord proxies the image when the message posts.
