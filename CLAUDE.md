...
The crowning announcement is built in `app/lib/clip-embeds.ts` (a pure leaf, same convention as `draft-embeds.ts`). The registered-role ping lives in the message `content`, never the embed — a mention inside an embed notifies nobody — with `allowed_mentions: { parse: [], roles: [roleId] }`, since a clip title is player-supplied and would otherwise ping the server through the bot. `clips.clip_of_week_number` is a running count that never resets, stored on the winning clip so past winners keep their number; a read error on that column means the migration hasn't been run and the heading drops the week rather than failing the update that archives the winner. The embed's image comes from `clipPreviewImageUrl`, which derives it from the YouTube video ID: `clips.thumbnail_url` is only populated for the link-only platforms, which are exactly the ones barred from winning, so it is null for every possible winner.
