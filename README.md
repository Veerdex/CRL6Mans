...
Stores the **Discord role ID** every player on a team holds while an event is running — 1v1, 2v2 and 3v3 alike. It's granted when teams are formed and stripped when the event ends. At 2v2/3v3 players hold it *alongside* their team role; at 1v1 it replaces the team role entirely, since a 1v1 "team" is one player and per-team roles would mean one role per entrant. If not set, the bot falls back to resolving a role named `Tournament` by name, creating it if it doesn't exist. Storing the role ID means the role can be renamed in Discord without the bot minting a duplicate.

| Option | Description |
|--------|-------------|
| `role` | The shared role for everyone in the running event |
...
- **The shared event role:** everyone on a team gets one role marking them as being in the running event, at every team size — link it with `/settournamentid`, or let the bot auto-create one named `Tournament`. It's added when teams are formed and stripped when the event ends. At 2v2/3v3 it sits alongside the team role; at `team_size = 1` it takes the place of one, since a team is one player there and per-team roles would mean one role per entrant.
